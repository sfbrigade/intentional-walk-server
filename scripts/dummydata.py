...
class SQLGenerator:
    ...
    def make_accounts(self, n: int) -> list[list[str], list[int]]:
        """
    ...
    def make_contests(
        self,
        n: int,
    ) -> list[list[str], list[int], dict[int, datetime]]:
...
def main() -> int:
    ...
    p.add_argument(
        ...
        default=250,
        type=int,
        help="number of walker accounts to generate",
    )
    args = p.parse_args()
