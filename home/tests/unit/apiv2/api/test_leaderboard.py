...
class TestLeaderboard(TestCase):
    ...
    def test_get_leaderboard(self):
        ...
        response = self.client.get("/api/v2/leaderboard/get?" + query)
        # Check for a successful response by the server
        self.assertEqual(response.status_code, 200)
        response_data = response.json()

        # Validate that tester account ({'account_id': 16, 'steps': 15000} isn't included)
        self.assertNotEqual(response_data["leaderboard"][0]["steps"], 15000)

        # Validate 10 Leaderboard entries received as user is in top 10
        self.assertEqual(response.status_code, 200)
        listlength = len(response_data["leaderboard"])
        ...
        listlength = len(response_data_pretest["leaderboard"])
        ...
        self.assertEqual(
            ...
            response_data_pretest["leaderboard"],
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_leaderboard_missing_contest_id(self):
        ...
        self.assertIn(
            "contest_id", response_data["detail"][0]["loc"], msg=fail_message
        )
        self.assertEqual(
    ...
    def test_get_leaderboard_invalid_contest_id(self):
        ...
        query = urllib.parse.urlencode(data)
        response = self.client.get("/api/v2/leaderboard/get?" + query)
        # Check for a 404 Not Found response by the server
    ...
    def test_get_leaderboard_invalid_device_id(self):
        ...
        self.assertEqual(
            response_data["detail"],
            f"Unregistered device - {data['device_id']}. Please register first!",
            msg=fail_message,
        )
