...
class TestContest(TestCase):
    ...
    def test_validates_dates(self):
        ...
        with self.assertRaises(ValidationError) as error:
            contest.save()
        self.assertEqual(
            error.exception.message, "End of contest must be after Start"
        )
    ...
    def test_active(self):
        ...
        self.assertIsNone(
            Contest.active(for_date=date(3000, 8, 14), strict=True)
        )
    ...
    def test_for_baseline(self):
        ...
        self.assertEqual(
            str(contest1.pk), Contest.for_baseline("3000-04-01").pk
        )
