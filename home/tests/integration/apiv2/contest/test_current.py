...
class ApiTestCase(TestCase):
    def setUp(self):
        ...
        self.url = "/api/v2/contest/current"
        # Content type
    ...
    def test_contest_current(self):
        ...
        with freeze_time("2020-04-28"):
            ...
            response_data = response.json()
            fail_message = f"Server response - {response_data}"
            self.assertEqual(response_data["contest_id"], str(contest1.pk))
            self.assertEqual(response_data["start_promo"], "2020-04-24")
            self.assertEqual(response_data["start"], "2020-05-01")
            self.assertEqual(response_data["end"], "2020-05-31")
        ...
        with freeze_time("2020-06-28"):
            ...
            response_data = response.json()
            fail_message = f"Server response - {response_data}"
            self.assertEqual(response_data["contest_id"], str(contest2.pk))
            self.assertEqual(response_data["start_promo"], "2020-06-21")
            self.assertEqual(response_data["start"], "2020-07-01")
            self.assertEqual(response_data["end"], "2020-07-31")
