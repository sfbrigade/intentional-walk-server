...
class ApiTestCase(TestCase):
    def setUp(self):
        ...
        self.request_params1.update({"name": "John Doe", "email": self.email1, "account_id": self.account_id1})
        self.request_params2.update({"name": "Joe Doe", "email": self.email2, "account_id": self.account_id2})
    ...
    def tearDown(self) -> None:
        for acct_id in self.existing_user_accts:
            response = self.client.delete(
                path=self.del_url, data={"account_id": acct_id}, content_type=self.content_type)
            self.check_delete_success(response, self.account_id1)

    def create_user_and_confirm_response(self, request_params):
        # Create the user
        response = self.client.post(path=self.create_url, data=request_params, content_type=self.content_type)
    ...
    def check_delete_success(self, response, deleted_account_id):
        ...
        fail_message = f"Server response - {response_data}"
        self.assertEqual(response_data["status"], "success", msg=fail_message)
        self.assertEqual(response_data["message"], "Account deleted successfully", msg=fail_message)

        # Check user/device no longer exists by trying & failing to update the nonexistent user
        with self.assertRaises(ObjectDoesNotExist):
            self.client.put(
                path=self.create_url, data={"account_id": deleted_account_id}, content_type=self.content_type)
    ...
    def check_users_still_exist(self, valid_account_ids=[]):
        ...
        for acct_id in valid_account_ids:
            expected_success_update_response = self.client.put(
                path=self.create_url, data={"account_id": acct_id}, content_type=self.content_type)
            # Check for a successful response by the server
    ...
    def test_delete_user_success(self):
        ...
        response = self.client.delete(
            path=self.del_url, data={"account_id": self.account_id1}, content_type=self.content_type)
    ...
    def check_delete_failure(self, response, expected_msg):
        ...
        self.assertEqual(response_data["message"], expected_msg, msg=fail_message)

    def test_delete_user_failure_nonexistent(self):
        # Delete nonexistent user
        response = self.client.delete(
            path=self.del_url, data={"account_id": "fakeID"}, content_type=self.content_type)

        self.check_delete_failure(response, expected_msg="Cannot find user with specified account id.")
    ...
    def test_delete_user_failure_incorrect_params(self):
        ...
        response = self.client.delete(
            path=self.del_url, data={"what_is_this": self.account_id1}, content_type=self.content_type)

        self.check_delete_failure(response, expected_msg="Required input 'account_id' missing in the request")
        self.check_users_still_exist([self.account_id1, self.account_id2])
