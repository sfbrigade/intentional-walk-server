...
class ApiTestCase(TestCase):
    def setUp(self):
        ...
        self.intentional_walks = {
            ...
            "intentional_walks": [
                ...
                {
                    "event_id": "2222",
                    "start": "2020-02-21T15:20:00-05:00",
                    "end": "2020-02-21T15:50:00-05:00",
                    "steps": 500,
                    "distance": 0.4,
                    "pause_time": 53.5,
                },
                {
                    "event_id": "3333",
                    "start": "2020-02-22T13:15:00-05:00",
                    "end": "2020-02-22T13:45:00-05:00",
                    "steps": 1000,
                    "distance": 0.7,
                    "pause_time": 100.1,
                },
        ...
        response = self.client.post(
            path="/api/intentionalwalk/create", data=self.intentional_walks, content_type="application/json",
        )
        # Check for a successful response by the server
        self.assertEqual(response.status_code, 200)
        # Parse the response
    ...
    def test_intentionalwalk_get(self):
        ...
        self.assertEqual(
            ...
            sum([dw["steps"] for dw in self.intentional_walks["intentional_walks"]]),
            msg=fail_message,
        )
        self.assertEqual(
            response_data["total_distance"],
            sum([dw["distance"] for dw in self.intentional_walks["intentional_walks"]]),
            msg=fail_message,
        )
        self.assertEqual(
            response_data["total_pause_time"],
            sum([dw["pause_time"] for dw in self.intentional_walks["intentional_walks"]]),
            msg=fail_message,
        )
        self.assertEqual(
            response_data["total_walk_time"],
            sum(
                [
                    (parser.parse(dw["end"]) - parser.parse(dw["start"])).total_seconds() - dw["pause_time"]
        ...
        self.assertEqual(
            len(response_data["intentional_walks"]), len(self.intentional_walks["intentional_walks"]), msg=fail_message
        )
    ...
    def test_intentionalwalk_get_aggregated(self):
        ...
        new_intentional_walks = {
            ...
            "intentional_walks": [
                {
                    "event_id": "4444",
                    "start": "2020-02-21T18:15:00-05:00",
                    "end": "2020-02-21T18:45:00-05:00",
                    "steps": 700,
                    "distance": 0.7,
                    "pause_time": 600,
                },
                {
                    "event_id": "8888",
                    "start": "2020-02-23T15:20:00-05:00",
                    "end": "2020-02-23T15:50:00-05:00",
                    "steps": 900,
                    "distance": 0.8,
                    "pause_time": 400,
                },
        ...
        response = self.client.post(
            path="/api/intentionalwalk/create", data=new_intentional_walks, content_type="application/json",
        )
        # Check for a successful response by the server
        self.assertEqual(response.status_code, 200)
        # Parse the response
        ...
        new_intentional_walks = {
            ...
            "intentional_walks": [
                {
                    "event_id": "9999",
                    "start": "2020-02-24T13:15:00-05:00",
                    "end": "2020-02-22T13:45:00-05:00",
                    "steps": 1000,
                    "distance": 1.1,
                    "pause_time": 600.5,
