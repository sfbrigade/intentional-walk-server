...
def validate_account_input(data: dict):
    ...
    Validation of account data input:
        ...
        * race_other:           must be specified when race includes 'OT'
        * gender:               value must be in GenderLabels
        * gender_other:         must be specified when gender is 'OT'
        * sexual_orien:         value must be in SexualOrientationLabels
        * sexual_orien_other:   must be specified when sexual_orien is 'OT'
    ...
    if (
        data.get("zip") is not None
    ):  # Required field but existence checked in validate_request_json
        assert (
            len(data["zip"]) >= 5 and len(data["zip"]) <= 10
        ), "Invalid zip code"
    if (
        data.get("age") is not None
    ):  # Required field but existence checked in validate_request_json
...
class AppUserCreateView(View):
    ...
    If present, the same endpoint will update user details except email.
    """
    ...
    def put(self, request, *args, **kwargs):
        ...
        # Validate json. If account_id is missing, send back the response
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Validate json. If any field is missing, send back the response
        json_status = validate_request_json(
            json_data,
            required_fields=["name", "email", "zip", "age", "account_id"],
        )
        if "status" in json_status and json_status["status"] == "error":
            return JsonResponse(json_status)
        ...
        try:
            ...
            if device.account.email.lower() != json_data["email"].lower():
                return JsonResponse(
                    {
                        "status": "error",
                        "message": "Email cannot be updated. Contact admin",
                    }
                )
        ...
        # To validate, retrieve the object back and send the details
        try:
            device = Device.objects.get(device_id=json_data["account_id"])
        except ObjectDoesNotExist:
            return JsonResponse(
                {
                    "status": "error",
                    "message": (
                        "App User failed to save successfully. Please retry"
                    ),
                }
            )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Cannot find user with specified account id.",
                }
            )
